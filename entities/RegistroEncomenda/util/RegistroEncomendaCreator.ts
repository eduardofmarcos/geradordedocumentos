export class RegistroEncomendaCreator {
  ...
  public static RegistroEncomendaValidator(
    valueToCheck: string
  ): Promise<boolean> {
  ...
  private static validarRegistroEncomenda(valueToCheck: any) {
    ...
    const RegistroEncomendaResult =
      this.gerarRegistroEncomenda(RegistroEncomenda);
    const clearRegistroEncomendaResult = RegistroEncomendaResult.replace(
      /\D/g,
      ""
    );
